...
const HistoryPage = {
  ...
  async exportCSV() {
    try {
      ...
      const result = await API.get('/checklists?limit=1000');
      ...
      const headers = ['ID', 'Status', 'Tipo', 'Placa', 'Modelo', 'Destino', 'Motorista', 'Vistoriador', 'Data Entrada', 'KM Entrada', 'Data Saída', 'KM Saída', 'KM Rodado', 'Criado Em'];
      const rows = data.map(c => [
        c.id, c.status, c.tipo, c.veiculo_placa, c.veiculo_modelo,
        c.destino, c.motorista_nome, c.vistoriador_nome,
        c.entrada_data, c.entrada_km, c.saida_data, c.saida_km,
        c.entrada_km && c.saida_km ? (parseInt(c.saida_km) - parseInt(c.entrada_km)) : '',
        c.criado_em
      ]);
      ...
      a.href = url;
      a.download = `checklists_export_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
